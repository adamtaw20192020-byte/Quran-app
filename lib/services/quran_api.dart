...
class QuranApiService {
  ...
  /// يجلب آيات سورة معيّنة مع النص العربي، ويدمج معها الترجمة إن طُلبت
  Future<List<Ayah>> getSurahAyahs(
    int surahNumber, {
    bool withTranslation = true,
    String translationEdition = defaultTranslationEdition,
  }) async {
    final editions = withTranslation
        ? '$quranEdition,$translationEdition'
        : quranEdition;
    ...
    if (withTranslation && data.length > 1) {
